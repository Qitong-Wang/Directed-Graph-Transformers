...
class MALNETSubDataset(DatasetBase):
    def __init__(self, 
                 dataset_path         ,
                 upto_hop,
                 dataset_name = 'MALNETSub',
                 **kwargs
                 ):
    ...
    def dataset(self):
        try:
            ...
            path = "./raw_data/MALNETsub_dataset.pkl"
            if self.split =="training":
                with open(path, 'rb') as file:
                    self._dataset = pickle.load(file)["train"]
            elif self.split == "validation":
                with open(path, 'rb') as file:
                    self._dataset = pickle.load(file)["val"]
    ...
    def read_record(self, token):
        ...
        #graph['edge_features'] = np.ones((num_edges,1)).astype(np.float32)
        #graph['node_features'] = np.ones((num_nodes, 1)).astype(np.float32)
...
class MALNETSubGraphDataset(GraphDataset,MALNETSubDataset):
    pass

class MALNETSubPEGraphDataset(PEEncodingsGraphDataset,MALNETSubDataset):
    pass

class MALNETSubStructuralGraphDataset(StructuralDataset,MALNETSubGraphDataset):
    pass

class MALNETSubStructuralPEGraphDataset(StructuralDataset,MALNETSubPEGraphDataset):
    pass
