...
class TrainingBase:
    def __init__(self, config=None, ddp_rank=0, ddp_world_size=1):
        ...
        self.stop_learning_count = 0
        self.minimal_val = 0.0
    ...
    def val_dataloader(self):
        ...
        if self.config['scheme'] == "MALNETSub":
            ...
            print("./lib/training/training.py L 147 Set val batchsize to 4 in MALNETSub")
        if not self.is_distributed:
            dataloader = DataLoader(**common_kwargs, 
    ...
    def model(self):
        ...
        if self.is_distributed:
            model = torch.nn.parallel.DistributedDataParallel(model,device_ids=[self.ddp_rank],
                                                              output_device=self.ddp_rank,find_unused_parameters=True)
        return model
    ...
    def prediction_epoch(self, epoch, logs):
        ...
        if self.is_main_rank:
            ...
            if results['val_acc'] < self.minimal_val:
                self.stop_learning_count = self.stop_learning_count + 1
                print("Worse result on valid! Now stop learning count is "+str(self.stop_learning_count))
            else:
                self.stop_learning_count = 0
                self.minimal_val = results['val_acc']
                print("Better result. Save predictions! Current val is "+str(self.minimal_val))
                self.save_predictions('validation', predictions)
                self.save_checkpoint()
            if self.stop_epochs > 0 and  self.stop_learning_count >= self.stop_epochs:
                print("Stop learning!")
